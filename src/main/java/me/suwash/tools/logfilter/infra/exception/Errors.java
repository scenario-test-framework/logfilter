...
public class Errors extends VndErrors implements Iterable<VndError> {
    ...
    /** エラー。 */
    private transient VndErrors errors;
     ...
     */
    public Errors(final LogFilterException exception) {
        super();
    ...
    public VndErrors add(final VndError error) {
        if (error == null) {
            throw new LogFilterException("check.notNull", new Object[] {
                "error"
            });
        }
     ...
     */
    protected LogFilterMessageSource getMessageSource() {
    ...
    public <T> void addViolations(final Set<ConstraintViolation<T>> violations) {
        ...
        for (final ConstraintViolation<T> curViolation : violations) {
            ...
            this.add(newVndError(
                ...
                getMessageSource().getMessage(Const.MSGCD_WRAP_VALIDATE_ERROR, new Object[] { curMessage, invalidValue})
            ));
        }
    }
